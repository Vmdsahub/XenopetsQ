...
export const PetPortrait: React.FC<PetPortraitProps> = ({ pet }) => {
  ...
  const totalSecondaryStats = pet.strength + pet.dexterity + pet.intelligence + 
                              pet.speed + pet.attack + pet.defense + 
                              pet.precision + pet.evasion + pet.luck;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {pet.conditions.length > 0 && (
          ...
          >
            {pet.conditions.map((condition, index) => (
              ...
              >
                <span className="text-sm">{getConditionIcon(condition.type)}</span>
              </motion.div>
            ))}
          </motion.div>
        )}
        ...
        >
          <span className="text-xs font-medium text-purple-700">{pet.personality}</span>
        </motion.div>
      </motion.div>
      ...
      >
        <div className={`text-center p-4 rounded-2xl border-2 ${getAttributeBgColor(pet.health)}`}>
          <Heart className={`w-8 h-8 mx-auto mb-2 ${getAttributeColor(pet.health)}`} />
          <p className="text-xs text-gray-600 font-medium">Health</p>
          <p className={`text-xl font-bold ${getAttributeColor(pet.health)}`}>{pet.health}/10</p>
        </div>
        <div className={`text-center p-4 rounded-2xl border-2 ${getAttributeBgColor(pet.happiness)}`}>
          <Smile className={`w-8 h-8 mx-auto mb-2 ${getAttributeColor(pet.happiness)}`} />
          <p className="text-xs text-gray-600 font-medium">Happiness</p>
          <p className={`text-xl font-bold ${getAttributeColor(pet.happiness)}`}>{pet.happiness}/10</p>
        </div>
        <div className={`text-center p-4 rounded-2xl border-2 ${getAttributeBgColor(pet.hunger)}`}>
          <Utensils className={`w-8 h-8 mx-auto mb-2 ${getAttributeColor(pet.hunger)}`} />
          <p className="text-xs text-gray-600 font-medium">Hunger</p>
          <p className={`text-xl font-bold ${getAttributeColor(pet.hunger)}`}>{pet.hunger}/10</p>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-1">
            ...
            <span className="text-sm font-bold text-yellow-600">{totalSecondaryStats}</span>
          </div>
        </div>
