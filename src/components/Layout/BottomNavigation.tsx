...
const navigationItems = [
  ...
  { id: 'inventory', label: 'Inventário', icon: Package, color: 'text-green-500' },
...
export const BottomNavigation: React.FC = () => {
  ...
  const items = user?.isAdmin 
    ? [...navigationItems, { id: 'admin', label: 'Admin', icon: Shield, color: 'text-red-500' }]
    : navigationItems;

  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-gray-100 shadow-lg">
      <div className={`flex justify-around max-w-md mx-auto px-2 py-2 ${user?.isAdmin ? 'grid grid-cols-5' : ''}`}>
