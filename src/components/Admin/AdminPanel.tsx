...
export const AdminPanel: React.FC = () => {
  ...
  const handleCreateCode = () => {
    ...
    if (allCodes.some(c => c.code.toLowerCase() === newCodeData.code.toLowerCase())) {
      addNotification({
        type: 'error',
        title: 'Erro',
  ...
  const tabs = [
    { id: 'codes', name: 'Códigos de Resgate', icon: Ticket, count: allCodes.length },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl shadow-xl p-6 border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-xl font-bold text-gray-900">Códigos de Resgate</h3>
            <p className="text-gray-600">Gerencie códigos promocionais e recompensas</p>
          </div>
          <motion.button
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-3 bg-blue-50 rounded-xl border border-blue-200">
            <p className="text-2xl font-bold text-blue-800">{allCodes.length}</p>
            <p className="text-xs text-blue-600">Total de Códigos</p>
          </div>
          <div className="text-center p-3 bg-green-50 rounded-xl border border-green-200">
            <p className="text-2xl font-bold text-green-800">{activeCodes.length}</p>
      ...
      {/* Codes List */}
      <div className="bg-white rounded-3xl shadow-xl p-6 border border-gray-100">
        <h4 className="text-lg font-bold text-gray-900 mb-4">Todos os Códigos</h4>
            ...
            {allCodes.map((code, index) => (
              <motion.div
                ...
                className={`p-4 rounded-2xl border-2 transition-all ${
                  code.isActive ? 'border-green-300 bg-green-50' : 'border-gray-300 bg-gray-50'
                }`}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <h5 className="font-bold text-gray-900 font-mono text-lg">{code.code}</h5>
                      <motion.button
                ...
                {/* Rewards */}
                <div className="mb-3">
                  <p className="text-sm font-medium text-gray-700 mb-2">Recompensas:</p>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {code.rewards.collectibles && code.rewards.collectibles.length > 0 && (
                ...
                <div className="flex items-center justify-between text-sm text-gray-600">
                  <span>
                    Usos: {code.currentUses}{code.maxUses !== -1 ? `/${code.maxUses}` : ' (ilimitado)'}
                  </span>
    ...
    <div className="max-w-md mx-auto space-y-6">
      ...
      >
        ...
        <div className="flex items-center space-x-2 p-3 bg-red-50 rounded-xl border border-red-200">
          ...
          <span className="text-red-700 font-medium text-sm">Acesso de Administrador</span>
        </div>
      </motion.div>
        ...
        <div className="p-6">
          ...
          {activeTab === 'items' && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Gerenciamento de itens em desenvolvimento.</p>
            </div>
          )}
          {activeTab === 'achievements' && (
            <div className="text-center py-8">
              <Trophy className="w-12 h-12 text-gray-400 mx-auto mb-3" />
              <p className="text-gray-600">Gerenciamento de conquistas em desenvolvimento.</p>
            </div>
          )}
        </div>
      </motion.div>
            ...
            >
              <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold text-gray-900">Criar Novo Código</h3>
                  <button
                    onClick={() => setShowCreateCodeModal(false)}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs text-gray-600 mb-1">Xenocoins</label>
                          <input
                            type="number"
                            value={newCodeData.rewards.xenocoins}
                            onChange={(e) => setNewCodeData(prev => ({
                              ...prev,
                              rewards: { ...prev.rewards, xenocoins: parseInt(e.target.value) || 0 }
                            }))}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                            min="0"
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setNewCodeData(prev => ({
                              ...prev,
                              rewards: { ...prev.rewards, cash: parseInt(e.target.value) || 0 }
                            }))}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                            min="0"
                          />
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">Pontos de Conta</label>
                        <input
                          type="number"
                          value={newCodeData.rewards.accountPoints}
                          onChange={(e) => setNewCodeData(prev => ({
                            ...prev,
                            rewards: { ...prev.rewards, accountPoints: parseInt(e.target.value) || 0 }
                          }))}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                          min="0"
                        />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewCodeData(prev => ({ ...prev, maxUses: parseInt(e.target.value) || -1 }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setNewCodeData(prev => ({ ...prev, isActive: e.target.checked }))}
        ...
        {editingCode && (
          <>
            ...
            >
              <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold text-gray-900">Editar Código</h3>
                  <button
                    onClick={() => setEditingCode(null)}
                    className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
                  >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditingCode(prev => prev ? { ...prev, name: e.target.value } : null)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                    />
                  </div>
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-sm font-medium text-gray-700 mb-2">Estatísticas de Uso</p>
                    <div className="text-sm text-gray-600 space-y-1">
                      <p>Usos atuais: {editingCode.currentUses}</p>
                      <p>Máximo de usos: {editingCode.maxUses === -1 ? 'Ilimitado' : editingCode.maxUses}</p>
                      <p>Criado em: {editingCode.createdAt.toLocaleDateString()}</p>
                      {editingCode.expiresAt && (
                        <p>Expira em: {new Date(editingCode.expiresAt).toLocaleDateString()}</p>
                      )}
                    </div>
                  </div>
                </div>
